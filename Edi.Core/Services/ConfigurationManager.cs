...
namespace Edi.Core.Services
{
    ...
    public class ConfigurationManager
    {
        ...
        private Dictionary<string, JObject> LoadCombinedConfigurations()
        {
            ...
            // Cargar configuraciones del directorio actual. A malformed game EdiConfig.json (e.g. an
            // older EDI mangled a device name) must NOT crash the app — fall back to defaults for it.
            if (File.Exists(_gameConfigPath))
            {
                ...
                {
                    System.Diagnostics.Debug.WriteLine($"Ignoring malformed game config '{_gameConfigPath}': {ex.Message}");
                }
            }
            ...
            if (File.Exists(_userConfigPath))
            {
                ...
                {
                    System.Diagnostics.Debug.WriteLine($"Ignoring malformed user config '{_userConfigPath}': {ex.Message}");
                }
            }
            else
            {
        ...
        private void Save(string typeName, object config)
        {
            ...
            if (File.Exists(targetPath))
            {
                ...
                catch { existingConfigs = new Dictionary<string, JObject>(); }   // corrupt target → rewrite fresh (self-heals)
            }
