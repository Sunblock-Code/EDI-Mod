...
namespace Edi.Core.Device.Handy
{
    public class HandyProvider : IDeviceProvider
    {
        ...
        private async Task Connect(string key)
        {
            ...
            // Skip work entirely if we already have a device for this key.
            // This is checked again inside the lock below to close the race window.
            lock (devices)
            {
                if (devices.ContainsKey(key))
                {
                    _logger.LogInformation($"Skip connect; device for Key: {key} already loaded.");
                    return;
                }
            }
            ...
            lock (devices)
            {
                // Re-check inside lock — another parallel Connect may have raced in.
                if (!devices.ContainsKey(key))
                {
                ...
                {
                    _logger.LogInformation($"Race won by another thread for Key: {key}; discarding duplicate device.");
                }
            }
