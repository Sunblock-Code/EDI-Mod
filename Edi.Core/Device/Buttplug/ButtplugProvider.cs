...
namespace Edi.Core.Device.Buttplug
{
    public class ButtplugProvider : IDeviceProvider
    {
        ...
        public async Task Connect()
        {
            ...
            try
            {
                if (!string.IsNullOrEmpty(Config.Url))
                {
                    ...
                    // Guard: only dial the websocket if Intiface is actually listening. Attempting a
                    // connect to a closed port can surface a fault that's NOT a ButtplugClientConnectorException
                    // (sometimes off the connector's own threads), which has torn the whole app down ~¼s later.
                    // Most users drive a Handy/OSR and never run Intiface, so this path was firing constantly.
                    if (!IsPortOpen(uri.Host, uri.Port, 350))
                    {
                        _logger.LogInformation($"Intiface/Buttplug not listening at {uri.Host}:{uri.Port}; skipping connect (reconnect timer will retry).");
                    }
                    else
                    {
            ...
            catch (Exception ex)   // widen: NO connect failure should be allowed to escape and kill EDI
            {
                _logger.LogError($"Failed to connect to Buttplug client: {ex.Message}");
            }
            finally
            {
        ...
        // Fast TCP reachability probe (localhost refuses instantly when nothing's listening).
