...
namespace Edi.Core.Device.Simulator
{
    ...
    public class PreviewDevice : DeviceBase<FunscriptRepository, FunscriptGallery>, IRange
    {
        ...
        // The funscript gallery currently playing (its Commands draw the line; Duration scales time).
        // Changes reference on each swap, so the preview can detect swaps by polling.
        public FunscriptGallery CurrentGallery => currentGallery;
        ...
        private async Task UpdateProgressBar()
        {
            ...
            // Map the 0-100 script value into the live output range [Min,Max] — the same scaling the
            // real toys apply — so the visualizer's dot reflects Intensity/Vibration (and rides the line).
            double scaled = Min + (Max - Min) * (Math.Clamp(interpolatedPosition, 0, 100) / 100.0);
            ProgressValue = Math.Clamp((int)Math.Round(scaled), 0, 100);
